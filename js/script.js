...
$document.ready(function () {
	...
	$('form.rd-mailform2').validate({  //   Проверка формы 
	       ...
	       messages: {		innerx: {   required:"Укажите длину в см!", number:"Только цифры!" },innery: {  required:"Укажите ширину в см!", number:"Только цифры!" },innerz: {  required:"Укажите высоту в см!", number:"Только цифры!" } },
			...
			if(  $(this).attr('name')=='submitted'  ){            // Форма прошла валидацию 
			 ...
			 $.getJSON('/bat/searchbase.php?callback=?',{act:'searchcase',innerx: $(this).find('input[name="innerx"]').val(),innery: $(this).find('input[name="innery"]').val(),innerz: $(this).find('input[name="innerz"]').val()},function(cases){
			  ...
			  if(cases.second_num > 0 ){	$('.caseselect').append( '<div class="partition">Варианты с большим запасом: '+cases.second_num+'шт.</div>');   } 
